...
- `By default, <product> expects users to own configuration directories and files that control how <product> runs.`
- `<product> is a service networking solution.`
- `<product> solves service networking issues.`

**Don't**

- `Next, the service will be registered.`
- `It is recommended to configure VCS access when first setting up an organization.`
- `By default, <product> expects that the configuration directory and files that run <product> are owned by the user.`
...
- ``To specify the location of a configuration file, use the `-config` flag with `<product> server`.``
- ``Open `main.conf` in your editor.``
...
- ``You must provide credentials to access <product>.`` 
